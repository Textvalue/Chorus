...
type OrgRow = {
  ...
  owner_member_id: string | null; logo_url: string | null;
};
function toOrg(r: OrgRow): Org {
  return {
    ...
    owner_member_id: r.owner_member_id, logo_url: r.logo_url ?? null,
  };
}
...
export async function saveOrg(org: Org): Promise<Org> {
  await query(
    `insert into orgs (org_id, name, website, positioning, icp, competitors, brand_dna, owner_member_id, logo_url, updated_at)
     ...
     on conflict (org_id) do update set
       ...
       owner_member_id=excluded.owner_member_id, logo_url=excluded.logo_url, updated_at=now()`,
    [org.org_id, org.name, org.website, org.positioning, J(org.icp), J(org.competitors),
     J(org.brand_dna), org.owner_member_id, org.logo_url ?? null]
  );
...
export async function upsertMember(member: Member): Promise<Member> {
  await query(
    `insert into members (member_id, org_id, name, headline, linkedin_url, voice_dna, prose_samples, expert_pov, profile_picture_url, updated_at)
     ...
     on conflict (member_id) do update set
       ...
       prose_samples=excluded.prose_samples, expert_pov=excluded.expert_pov,
    ...
    [member.member_id, member.org_id, member.name, member.headline, member.linkedin_url,
     J(member.voice_dna), J(member.prose_samples), J(member.expert_pov), member.profile_picture_url ?? null]
  );
...
export async function updateMember(
  ...
  await query(
    `update members set name=$2, headline=$3, linkedin_url=$4, voice_dna=$5::jsonb,
       prose_samples=$6::jsonb, expert_pov=$7::jsonb, profile_picture_url=$8, updated_at=now() where member_id=$1`,
    [memberId, next.name, next.headline, next.linkedin_url, J(next.voice_dna),
     J(next.prose_samples), J(next.expert_pov), next.profile_picture_url ?? null]
  );
...
// orgId is optional so background job runners (which have no request session) can pass the
// org explicitly; falls back to the session-scoped org for direct calls.
export async function setPostImage(postId: string, imageUrl: string, orgId?: string): Promise<void> {
  const oid = orgId ?? (await currentOrgId());
  if (!oid) return;
  await query("update posts set image_url = $1 where id = $2 and org_id = $3", [imageUrl, postId, oid]);
}

export async function setPostCarousel(postId: string, slides: Post["carousel"], orgId?: string): Promise<void> {
  const oid = orgId ?? (await currentOrgId());
  if (!oid) return;
  await query("update posts set carousel = $1::jsonb where id = $2 and org_id = $3", [J(slides), postId, oid]);
}

// ---- background jobs ----
// createJob / getJob run inside request handlers (session-scoped). updateJob runs inside the
// after() runner (no session) and keys on the unguessable job id alone.
type JobRow = {
  id: string; kind: JobKind; status: Job["status"];
  result: unknown | null; error: string | null; post_id: string | null;
};
function toJob(r: JobRow): Job {
  return {
    id: r.id, kind: r.kind, status: r.status,
    result: r.result ?? null, error: r.error ?? null, post_id: r.post_id ?? null,
  };
}

export async function createJob(kind: JobKind, input: unknown, postId?: string): Promise<string | null> {
  const orgId = await currentOrgId();
  if (!orgId) return null;
  const jobId = id("job");
  await query(
    `insert into jobs (id, org_id, kind, status, input, post_id) values ($1,$2,$3,'pending',$4::jsonb,$5)`,
    [jobId, orgId, kind, J(input), postId ?? null]
  );
  return jobId;
}
...
export async function getJob(jobId: string): Promise<Job | null> {
  const orgId = await currentOrgId();
  if (!orgId) return null;
  const rows = await query<JobRow>("select * from jobs where id = $1 and org_id = $2", [jobId, orgId]);
  return rows[0] ? toJob(rows[0]) : null;
}

export async function updateJob(
  jobId: string,
  fields: { status?: Job["status"]; result?: unknown; error?: string | null; post_id?: string | null }
): Promise<void> {
  const sets: string[] = ["updated_at = now()"];
  const vals: unknown[] = [];
  if (fields.status !== undefined) { vals.push(fields.status); sets.push(`status = $${vals.length + 1}`); }
  if (fields.result !== undefined) { vals.push(J(fields.result)); sets.push(`result = $${vals.length + 1}::jsonb`); }
  if (fields.error !== undefined) { vals.push(fields.error); sets.push(`error = $${vals.length + 1}`); }
  if (fields.post_id !== undefined) { vals.push(fields.post_id); sets.push(`post_id = $${vals.length + 1}`); }
  await query(`update jobs set ${sets.join(", ")} where id = $1`, [jobId, ...vals]);
}
